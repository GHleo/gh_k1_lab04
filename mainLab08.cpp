...
int main() {
    ...
    if ((inAB == 'B') || (inAB == 'b')) {
        ...
        cout << B << "  " << B[0] << "  " << B[2] << endl;// возвращает адрес памяти указателя ссылки из массива указателей В
        cout << B[0][0] << "  " << **B << "  " << *B[0] << endl;// первый элемент матрицы В.  B[0][0] - по явному адресу; *B[0] - через ссылку
        cout << *(*(B + 1)) << "  " << *B[1] << endl;// вывод элемента матрицы с адресом B[2:1] (2 - ссылка на первый элем. матрицы; 1 - ссылка на первый эл. массива + 1)
        cout << *(B[0] + 1) << "  " << *(*B + 1) << endl; //вывод элемента матрицы с адресом B[1:2] (1 - ссылка на первый элем. 2й строки матрицы; 2 - ссылка на первый эл. массива + 1)
        cout << B[0][20] << "  " << *(B[0] + 20) << "  " << *B[2] << endl;// вывод значения памяти вне размера матрицы
